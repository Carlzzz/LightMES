...
def test_api_pass_requires_login(client, db_session):
    resp = client.post("/api/production/pass",
        json={"work_station_id": 1, "work_order_code": "X"})
    assert resp.status_code == 401
...
def test_api_pass_success(client, db_session):
    ...
    _login(client, db_session)
    resp = client.post("/api/production/pass",
        json={"work_station_id": w1.id, "work_order_code": "PWO"})
    assert resp.status_code == 200
    body = resp.json()
    assert body["sn"] == "P001"
...
def test_api_pass_maps_domain_error_to_json(client, db_session):
    ...
    _login(client, db_session)
    # 用不属于工单产线的作业站过站 → BusinessRuleError(422)，全局 handler 映射为 JSON
    resp = client.post("/api/production/pass",
        json={"work_station_id": other_ws.id, "work_order_code": "PWO"})
    assert resp.status_code == 422
    body = resp.json()
    assert "detail" in body
    assert "不属于本工单产线" in body["detail"]


def test_scan_wrong_station_first_pass_rolls_back_orphan(client, db_session):
    """首件扫错作业站被拒后，已 flush 的 SerialUnit 必须回滚，不得留下孤儿。"""
    w1, w2, wo = _line_2step(db_session)
    wo_id = wo.id
    wo_code = wo.code
    _login(client, db_session)
